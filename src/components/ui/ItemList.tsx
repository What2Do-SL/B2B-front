...
export default function ItemList({
  ...
  return (
    <div className={`bg-white rounded-sm shadow-sm border border-gray-200 overflow-hidden ${className}`}>
      {/* Header */}
      <div className="bg-green-600 px-6 py-4 border-b border-gray-200">
        <div className="grid gap-4" style={{ gridTemplateColumns: `${columns.map(() => '1fr').join(' ')} auto` }}>
          {columns.map((column) => (
            <div key={column.key} className="text-sm font-medium text-beige">
              {column.label}
            </div>
          ))}
          <div className="text-sm font-medium text-beige">
            Acciones
          </div>
        </div>
      </div>
      ...
      <div className="divide-y divide-gray-200">
        {items.map((item, index) => (
          ...
          >
            <div className="grid gap-4 items-center" style={{ gridTemplateColumns: `${columns.map(() => '1fr').join(' ')} auto` }}>
              {columns.map((column) => (
